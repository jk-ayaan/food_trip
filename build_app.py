...
const state={sec:"food",q:"",cat:"전체",gu:"전체",sort:"def",view:"list",lang:(()=>{const s=localStorage.getItem("bf_lang");if(s)return s;const n=(navigator.language||"ko").slice(0,2);return ["en","ja","zh"].includes(n)?n:"ko"})()};
...
function filtered(){
  ...
  const s=state.sort;
  if(s==="view")list=[...list].sort((a,b)=>(b.v||0)-(a.v||0));
...
function card(r){
  ...
  ${tr(r.a)?`<div class="row">${PIN}<span>${esc(tr(r.a))}</span></div>`:""}
...
function popHtml(r){const tel=(r.p||"").replace(/[^0-9+]/g,"");const hl=highlight(r);const mapq=encodeURIComponent(tr(r.n)+" "+r.g);
  return `<div class="pop">${r.t?`<img class="pimg" src="${esc(r.t)}" alt="" onerror="this.style.display='none'">`:""}<div class="pbody"><div class="pn">${esc(tr(r.n))}</div>${hl&&hl.t?`<div class="pm">${esc(hl.t)}</div>`:""}<div class="pa">${esc(catName(r.c))}${r.g?" · "+esc(guName(r.g)):""}<br>${esc(tr(r.a))}</div><div class="pacts">${tel?`<a class="call" href="tel:${tel}">${U().call}</a>`:""}<a href="https://map.kakao.com/?q=${mapq}" target="_blank" rel="noopener">${U().mapBtn}</a><a href="${esc(r.u)}" target="_blank" rel="noopener">${U().detail}</a></div></div></div>`}
...
function applyLang(){const u=U();document.documentElement.lang=state.lang;
  ...
  document.getElementById("sort").innerHTML=Object.entries(u.sorts).map(([k,v])=>`<option value="${k}"${state.sort===k?" selected":""}>${v}</option>`).join("");
...
function setView(v){state.view=v;document.querySelectorAll("#viewtog button").forEach(b=>b.classList.toggle("on",b.dataset.v===v));
  ...
  render();if(v==="map"){const h=document.getElementById("maphint");h.classList.add("show");setTimeout(()=>h.classList.remove("show"),2600)}}
