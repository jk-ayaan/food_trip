...
.hero h1{margin:0;font-size:22px;font-weight:800;letter-spacing:-.02em;display:flex;align-items:center;gap:7px;white-space:nowrap}
@media(max-width:600px){
  .hero .topline{flex-wrap:wrap;row-gap:10px}
  .hero .topline>div:first-child{width:100%}
  .hero h1{font-size:20px}
  .hero .topline>div:last-child{margin-left:auto}
}
...
.mpanel{background:#fff;width:100%;max-width:480px;max-height:82vh;max-height:85dvh;border-radius:22px 22px 0 0;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 -10px 40px rgba(0,0,0,.25)}
@media(min-width:560px){.mlay{align-items:center;padding:20px}.mpanel{border-radius:22px}}
.mhead{display:flex;align-items:center;justify-content:space-between;padding:16px 18px 10px;flex:none}
...
.mlist{overflow:auto;padding:2px 12px calc(18px + var(--safe-b));min-height:0;-webkit-overflow-scrolling:touch}
...
.rpanel{max-width:580px}
#rmap{height:min(62vh,540px);width:100%;background:#dfe6ee;flex-shrink:1;min-height:220px}
.rhint{font-size:11.5px;color:var(--sub);font-weight:600;text-align:center;padding:10px 14px calc(14px + var(--safe-b))}
.rpin{transform:translate(-50%,-50%);display:inline-flex;align-items:center;gap:4px;background:#fff;border:1.5px solid var(--line);border-radius:999px;padding:5px 10px;font-size:12px;font-weight:800;color:var(--ink);box-shadow:0 3px 10px rgba(20,40,70,.22);white-space:nowrap;cursor:pointer}
...
.sbody{overflow:auto;padding:0 16px;flex:1;min-height:0;-webkit-overflow-scrolling:touch}
...
<div class="mlay" id="rlay">
  <div class="mpanel rpanel">
...
const RGT_D={ko:{title:"지도에서 지역 선택",hint:"라벨을 누르면 그 지역으로 이동해요 · 수도권을 확대하면 수원·안산·판교·정자·서현이 나와요"},
 ...
 ja:{title:"地図で地域を選択",hint:"ラベルをタップで移動 · 首都圏を拡大すると水原・安山・板橋・亭子・書峴が表示"},
 zh:{title:"在地图上选择地区",hint:"点击标签切换地区 · 放大首都圈可见水原·安山·板桥·亭子·书岘"}};
function RGT(){return RGT_D[state.lang]}
let rmap=null,rmain=null,rdetail=null;
const RDETAIL=new Set(["suwon","ansan","pangyo","jeongja","seohyeon"]);
function rlOpen(o){document.getElementById("rlay").classList.toggle("show",o);if(!o)return;
  document.getElementById("rlTitle").textContent="🗺️ "+RGT().title;
  document.getElementById("rlHint").textContent=RGT().hint;
  if(!rmap){rmap=L.map("rmap",{zoomControl:true,attributionControl:false}).setView([36.1,127.9],7);
    L.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",{subdomains:"abcd",maxZoom:15}).addTo(rmap);
    rmain=L.layerGroup().addTo(rmap);rdetail=L.layerGroup();
    rmap.on("zoomend",()=>{if(rmap.getZoom()>=9)rmap.addLayer(rdetail);else rmap.removeLayer(rdetail)})}
  rmain.clearLayers();rdetail.clearLayers();
  REGIONS.forEach(r=>{
